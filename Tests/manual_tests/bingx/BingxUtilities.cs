...
using manual_tests.bingx.Models;
using manual_tests.bingx.Providers;
using Serilog;

namespace manual_tests.bingx;

public class BingxUtilities
{
    public ILogger Logger { get; set; }

    public BingxUtilities(ILogger logger) => Logger = logger;
    ...
    public async Task CalculateFinancialPerformance(DateTime startDateTime, Trade trade)
    {
        ...
        HttpResponseMessage response = await trade.GetOrders(startDateTime);
        await EnsureSuccessfulBingxResponse(response);
