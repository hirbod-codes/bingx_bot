...
public class Trade : Api
{
    ...
    public async Task<HttpResponseMessage> GetOrders(DateTime startTime, DateTime? endTime = null) => await Utilities.HandleBingxRequest("https", Base_Url, "/openApi/swap/v2/trade/allOrders", "GET", ApiKey, ApiSecret, new
    {
        symbol = Symbol,
        // orderId = "1735954256061530112",
        // startTime = DateTimeOffset.Parse(startTime.ToString()).ToUnixTimeMilliseconds(),
        // endTime = endTime is null ?  DateTimeOffset.Parse(DateTime.UtcNow.ToString()).ToUnixTimeMilliseconds() : DateTimeOffset.Parse(endTime.ToString()!).ToUnixTimeMilliseconds(),
    });
