...
{
    public static IBroker CreateBroker(string brokerName, IBrokerOptions brokerOptions, ILogger logger, ITime time, ICandleRepository? candleRepository, IPositionRepository? positionRepository) => brokerName switch
    {
        BrokerNames.BINGX => new BingxBroker(brokerOptions, new BingxUtilities(logger), candleRepository!, logger, time),
