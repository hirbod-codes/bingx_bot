...
namespace CandleRepository.src.Configuration.Extensions.DockerSecrets;
...
public class DockerSecretsConfigurationProvider : ConfigurationProvider
{
    ...
    private void ProcessFile(string secretFilePath)
    {
        ...
        using (var reader = new StreamReader(_fileSystem.File.OpenRead(secretFilePath)))
        {
            ...
            {
                secretKey = TrimStart(secretKey, thisFilePrefix, StringComparison.InvariantCultureIgnoreCase);
                Data.Add(secretKey, secretValue);
            }
        }
    }
    private static string TrimStart(string source, string value, StringComparison comparisonType)
    {
