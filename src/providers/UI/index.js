...
firstData = data[data.length - 1].Date
lastData = data[0].Date

var trace2 = {
    x: data.map(elm => Date.parse(elm.Date)),
    open: data.map(elm => elm.Open),
    high: data.map(elm => elm.High),
    low: data.map(elm => elm.Low),
    close: data.map(elm => elm.Close),
...
console.log(trace2.open);
console.log(trace2.high);
console.log(trace2.low);
console.log(trace2.close);
...
var layout = {
    ...
    xaxis: {
        autorange: true,
        domain: [0, 1],
        range: [firstData, lastData],
        rangeslider: { range: [firstData, lastData] },
        title: 'Date',
        type: 'date'
    },
    yaxis: {
...
Plotly.newPlot('chart_tmp', data, layout);
